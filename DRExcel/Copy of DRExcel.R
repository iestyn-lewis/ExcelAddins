...
PerformDRAnalysis <- function(sampleid) {
    ...
    params.fixed = c(NA, lock.bottom, lock.top, NA)
    
    # make the graph and generate the model
    my.model <- doseResponseGraph(dr, x.name, y.name, series.name, dev="png", compound.name = paste(sampleid), y.axis.lim = c(y.min, y.max), x.axis.label = x.label, y.axis.label = y.label, size = c(graph.width, graph.height), show.rel.intcept = show.rel.intcept, show.abs.intcept = show.abs.intcept, fixed = params.fixed) 
    return (my.model)
}
